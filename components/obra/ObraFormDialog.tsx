...
export function ObraFormDialog({ open, onOpenChange, obra, onSuccess }: ObraFormDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (obra) {
        // Editar - NÃO atualiza status (será feito em componente separado)
        ...
        // Criar - status inicial sempre 'planejamento'
        obraData.created_by = user?.id
        obraData.status = 'planejamento'
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Para alterar o status da obra, use o botão "Alterar Status" na página de detalhes
            </p>
          </div>
