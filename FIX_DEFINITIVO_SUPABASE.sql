-- =====================================================
-- FIX DEFINITIVO - FORÇAR RECRIAÇÃO DA FUNÇÃO
-- Execute este arquivo COMPLETO no Supabase SQL Editor
-- =====================================================

-- PASSO 1: Deletar TODAS as versões da função (com e sem parâmetros)
DROP FUNCTION IF EXISTS arquivar_obra_no_memorial(UUID, UUID, TEXT) CASCADE;
DROP FUNCTION IF EXISTS arquivar_obra_no_memorial(UUID, UUID) CASCADE;
DROP FUNCTION IF EXISTS arquivar_obra_no_memorial(UUID) CASCADE;
DROP FUNCTION IF EXISTS arquivar_obra_no_memorial CASCADE;

-- PASSO 2: Recriar a função com SELECT simples (sem COALESCE para testar)
CREATE FUNCTION arquivar_obra_no_memorial(
  p_obra_id UUID,
  p_user_id UUID,
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM rdos WHERE obra_id = p_obra_id LIMIT 1) THEN
    ...
    SELECT
      ...
      id,
      data_relatorio,
      clima_manha_tempo || ' / ' || clima_noite_tempo,
      created_by,
      '', '', '', '',
  ...
  -- 6. Mover caixa de obra (TESTANDO SEM CATEGORIA PRIMEIRO)
  IF EXISTS (SELECT 1 FROM obra_caixa WHERE obra_id = p_obra_id LIMIT 1) THEN
    INSERT INTO memorial_caixa_obra (
      memorial_obra_id, movimentacao_original_id, tipo, descricao,
...
-- PASSO 3: Verificar se foi criada com sucesso
SELECT
  routine_name,
  routine_type,
