-- =============================================
-- MELHORIAS NO SISTEMA DE CHAT (VERSÃO SAFE)
-- - Verifica se tabelas/policies já existem antes de criar
-- =============================================
...
CREATE TABLE IF NOT EXISTS chat_tasks (
  ...
  status TEXT DEFAULT 'pending',
  parent_task_id UUID REFERENCES chat_tasks(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
...
-- =============================================
-- 11. HABILITAR REALTIME (SAFE - verifica se já existe)
-- =============================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
    AND schemaname = 'public'
    AND tablename = 'scheduled_meetings'
  ) THEN
